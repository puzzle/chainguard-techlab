...
```bash
docker build -t puzzle/chainguard-techlab .
```
...
```bash
docker run --rm -p 8080:8080 puzzle/chainguard-techlab
```
...
```bash
podman run --rm --rmi --publish 8080:8080 localhost/puzzle/chainguard-techlab
```
